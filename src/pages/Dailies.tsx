...
const initialDailies: DailyItem[] = [
  { productId: "p1", frequency: "daily", quantity: 1, active: true },
  { productId: "p4", frequency: "weekly", quantity: 2, active: true },
  { productId: "p11", frequency: "custom", quantity: 1, active: false, customDays: ["Mon", "Wed", "Fri"] },
];
    ...
    <div className="min-h-screen safe-bottom">
      <header className="sticky top-0 z-40 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3">
          <Link to="/profile" className="flex h-9 w-9 items-center justify-center rounded-xl bg-secondary lg:hidden">
      ...
      <main className="mx-auto max-w-6xl px-4 py-4 space-y-3 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0">
        {dailies.map(item => {
          ...
          return (
            <div key={item.productId} className={`glass-card p-4 space-y-3 transition-all ${!item.active ? "opacity-60" : ""}`}>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => toggle(item.productId)}
                  className={`flex h-9 w-9 items-center justify-center rounded-xl transition-colors ${
                    item.active ? "bg-primary/15 text-primary" : "bg-secondary text-muted-foreground hover:text-foreground"
                  }`}>
                ...
                {(["daily", "weekly", "custom"] as const).map(freq => (
                  <button key={freq} onClick={() => setFreq(item.productId, freq)}
                    className={`rounded-full px-3 py-1.5 text-[10px] font-medium capitalize transition-all ${
                      item.frequency === freq ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
                    }`}>
                    {freq}
                  </button>
                ))}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 rounded-xl bg-secondary p-0.5">
                  <button onClick={() => updateQty(item.productId, item.quantity - 1)}
                    className="flex h-7 w-7 items-center justify-center rounded-lg bg-background text-foreground hover:bg-card">
