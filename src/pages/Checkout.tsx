    ...
    <div className="min-h-screen safe-bottom">
      <header className="sticky top-0 z-40 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3">
          <Link to="/cart" className="flex h-9 w-9 items-center justify-center rounded-xl bg-secondary lg:hidden">
      ...
      <main className="mx-auto max-w-6xl px-4 py-4 pb-36 lg:grid lg:grid-cols-3 lg:gap-6">
              ...
              {deliverySlots.map(slot => (
                <button key={slot} onClick={() => setSelectedSlot(slot)}
                  className={`rounded-xl px-3 py-2.5 text-xs font-medium transition-all ${
                    selectedSlot === slot ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
                  }`}>
                  {slot}
                </button>
              ))}
            ...
            {paymentMethods.map(({ id, label, icon: Icon, desc }) => (
              <button key={id} onClick={() => setSelectedPayment(id)}
                className={`flex w-full items-center gap-3 rounded-xl p-3 transition-all ${
                  selectedPayment === id ? "bg-primary/10 ring-1 ring-primary" : "bg-secondary/50 hover:bg-secondary"
                }`}>
        ...
        <div className="mt-4 lg:mt-0">
          <div className="glass-card p-4 space-y-3 lg:sticky lg:top-20">
            ...
            <button onClick={handlePlaceOrder}
              className="hidden lg:flex w-full items-center justify-center rounded-2xl bg-primary py-4 text-sm font-bold text-primary-foreground transition-all active:scale-[0.98] glow-primary hover:bg-primary/90">
